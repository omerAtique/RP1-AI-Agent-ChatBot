    ...
    def _agentic_flow(self, query: str) -> str:
        ...
        try:
            ...
            for query in queries:
                retrieved_chunks = self.vector_store_services.retrieve_chunks(query)

                if not retrieved_chunks:
                    logger.warning("No chunks retrieved")
                    continue
                
                retrieved_chunks.extend(retrieved_chunks)
            ...
            for chunk in retrieved_chunks:
                context_string += f"- {chunk.page_content}\n"

            context = self.reranking_chunks._rerank_chunks_prepare_context(query, context_string, 'gpt-4.1-mini')

            if not context:
                logger.error("Error in _agentic_flow: Context preparation failed")
                return None
            
            final_response = self.query_response._final_query_response(query, context, 'gpt-4.1-mini')
